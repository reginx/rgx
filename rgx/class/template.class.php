...
class template extends rgx {
    ...
    public function parse_tpl ($tmpfile, $sfile) {
        ...
        $html = preg_replace_callback('/\{(\$[^\}]*?)\}/i',
                array($this, 'parse_var'), $html);
        // foreach
        $html = preg_replace_callback(
                '/\{\s*foreach\s+(\$[^\s]+)\s+(\$[\w]+)\s+(\$?[\w]+)\s*\}/i',
                array($this, 'parse_foreach'), $html);
        // endforeach
        ...
        $html = preg_replace_callback('/\{\s?__([\w\-_]*?)__\s?\}/is',
                array($this, 'parse_constant'), $html);
        ...
        // lang
        $html = preg_replace_callback('/\{lang\s*\:\s*(.*?)\}/is', array($this, 'parse_lang'), $html);
        // function
        $html = preg_replace('/\{\s*:(.*?)\s*?\}/i', '<?php echo(\1);?>', $html);
        // dynamic url
        $html = preg_replace('/\{\s?url\:\(?(.*?)\)?\}/i', '<?php echo(RGX\router::url(\\1)); ?>', $html);
        // static url
        ...
        $html = preg_replace('/\{\s?_([\w\-_\.]+?)_\s?\}/is',
                '<?php echo(RGX\CFG(strtolower("\1")) ?: strtolower("\1")); ?>',
                $html);
        // token
        ...
        $html = "<?php " .
                "namespace " . NS . ";use re\\rgx as RGX;" .
        ...
        foreach ((array) $tag as $k => $v) {
            ...
            if (empty($temp[1])) {
                switch ($temp[0]) {
                    // {$foo|thumb}
                    case 'thumb':
                        $var = "RGX\\image::get_thumb_name({$var})";
                        break;
                    // {$foo|date}
                    case 'date':
                        $var = "date('Y-m-d', {$var})";
                ...
                switch ($temp[0]) {
                    ...
                    case 'thumb':
                        if ($is_var) {
                            $var = "RGX\\image::get_thumb_name({$var} , {$temp[1]})";
                        }
                        else {
                            $var = "RGX\\image::get_thumb_name({$var} , '{$temp[1]}')";
                        }
                    ...
                    // {$foo|percent,$bar}
                    case 'percent':
                        $var = "RGX\\misc::pencent({$var} , {$temp[1]})";
                        break;
                    default:
                        throw new exception(
